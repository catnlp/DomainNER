...
from modules.seqmodel import SeqModel
import visdom
...
def train(data, name):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu)
            instance_count += 1
            loss, tag_seq = model.neg_log_likelihood_loss(batch_word, batch_features, batch_wordlen, batch_label, mask)
            ...
            if end % 100 == 0:
                ...
                losses.append(sample_loss / 100.0)
                Lwin = 'Loss of ' + name
        ...
        if total_loss > 1e8 or str(total_loss) == "nan":
            print("ERROR: LOSS EXPLOSION (>1e8) ! PLEASE SET PROPER PARAMETERS AND STRUCTURE! EXIT....")
            exit(0)
        speed, acc, p, r, f_train, _, _ = evaluate(data, model, "train")
        # continue
        ...
        if current_score > best_test:
            ...
            model_name = data.model_dir + '.' + name + ".model"
        ...
        print("Current best F1: ", best_test)
